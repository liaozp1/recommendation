...
def createFPTree(frozenDataSet, minSupport):
    ...
    headPointTable = {k: v for k, v in headPointTable.items() if v >= minSupport} ##minSupport过大会导致headPointTable={}
    ...
    for items, count in frozenDataSet.items():
        ...
        if len(frequentItemsInRecord) > 0:
            orderedFrequentItems = [v[0] for v in sorted(frequentItemsInRecord.items(), key=lambda v:(v[1], v[0]), reverse=True)]
...
def mineFPTree(headPointTable, prefix, frequentPatterns, minSupport):
    ...
    # print('headPointTable:', headPointTable)
    if headPointTable == None:
        return
...
def getRules(frequentset,currentset, rules, frequentPatterns, minConf):
    for frequentElem in currentset:
        ...
        frequentsetNums=frequentPatterns[frequentset]
        subsetNums=frequentPatterns[subSet]
        if (confidence >= minConf):
            ...
            for rule in rules:
                if(rule[0] == subSet and rule[1] == frequentset - subSet):
                    flag = True
            if(flag == False):
                rules.append((subSet, frequentset - subSet, confidence, frequentsetNums,subsetNums))
